...
enum GCJ02Converter {
    ...
    /// - Returns: `true` if the coordinate is within HK or Macau bounding box.
    static func isInSpecialRegion(lat: Double, lng: Double) -> Bool {
        // Hong Kong bounding box
        let isHK = (22.15...22.55).contains(lat) && (113.83...114.42).contains(lng)
        // Macau bounding box
        let isMacau = (22.10...22.22).contains(lat) && (113.52...113.60).contains(lng)
        return isHK || isMacau
    }
    ...
    static func wgs84ToGcj02(lat: Double, lng: Double) -> (lat: Double, lng: Double) {
        if isInSpecialRegion(lat: lat, lng: lng) { return (lat, lng) }
