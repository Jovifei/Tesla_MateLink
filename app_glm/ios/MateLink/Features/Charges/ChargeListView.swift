    ...
    var body: some View {
        NavigationStack {
            Group {
                ...
                else if charges.isEmpty { ContentUnavailableView("No Charges Yet", systemImage: "bolt.fill", description: Text("Plug in to start charging!")).padding() }
                else {
                    List(charges) { ch in
                        NavigationLink(destination: ChargeDetailView(charge: ch)) {
                            HStack {
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    ...
                                    Text("\(ch.chargeEnergyAdded, specifier: "%.1f") kWh · \(ch.startBatteryLevel)% → \(ch.endBatteryLevel ?? 0)%\(ch.cost > 0 ? " · \(ch.cost.formatted(.currency(code: Locale.current.currency?.identifier ?? "USD")))" : "")").font(.caption).foregroundColor(.secondary)
                                }
                            }
                        }
                    }.listStyle(.plain)
