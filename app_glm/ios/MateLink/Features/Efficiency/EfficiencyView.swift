...
struct EfficiencyView: View {
    ...
    @State private var goldenFootScore: Double = 0
                ...
                VStack(spacing: 16) {
                    ...
                    if goldenFootScore > 0 {
                        let (icon, label, color): (String, String, Color) = {
                            switch goldenFootScore {
                            case 90...: return ("🥇", "Excellent", Color(hex: "10B981"))
                            case 70..<90: return ("🥈", "Good", Color(hex: "3B82F6"))
                            case 50..<70: return ("🥉", "Fair", Color(hex: "F59E0B"))
                            default: return ("", "Needs Improvement", Color(hex: "EF4444"))
                            }
                        }()
                        HStack(spacing: 12) {
                            Text(icon).font(.system(size: 36))
                            VStack(alignment: .leading, spacing: 2) {
                                Text("Golden Foot Score").font(.caption).foregroundColor(.secondary)
                                Text("\(Int(goldenFootScore))").font(.title).bold().foregroundColor(color)
                            }
                            Spacer()
                            Text(label).font(.subheadline.weight(.semibold)).foregroundColor(color)
    ...
    func loadData() async {
        ...
        let actualConsumption = points.isEmpty ? 0.0 : Double(points.map(\.efficiency).reduce(0, +)) / Double(points.count)
        goldenFootScore = if actualConsumption <= 0 { 0.0 } else { min(100.0, max(0.0, (150.0 / actualConsumption) * 100.0)) }
    }
}
