...
struct CostView: View {
    ...
    @State private var tariffConfig: TariffConfig = {
        ...
        NavigationStack {
            ScrollView {
                VStack(spacing: 16) {
                    ...
                    // Summary Cards
                    HStack(spacing: 12) {
                        StatCard(title: "Total Cost", value: totalCost.formatted(.currency(code: Locale.current.currency?.identifier ?? "USD")), subtitle: "", color: .primary)
                        StatCard(title: "Home AC", value: acTotal.formatted(.currency(code: Locale.current.currency?.identifier ?? "USD")), subtitle: "", color: .blue)
                        StatCard(title: "DC Fast", value: dcTotal.formatted(.currency(code: Locale.current.currency?.identifier ?? "USD")), subtitle: "", color: .orange)
                    }.padding(.horizontal)
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        Text("Location Ranking (\(Locale.current.currencySymbol ?? "$")/kWh)")
                        ...
                        ForEach(Array(ranking.prefix(10).enumerated()), id: \.element.id) { i, loc in
                            HStack(spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 2) {
                                    ...
                                    if tariffConfig.isEnabled {
                                        Text(NSLocalizedString("cost.tou_estimate", comment: "") + " \u{00A5}\(loc.touCost.formatted(.number.precision(.fractionLength(2))))")
                                            .font(.caption2).foregroundColor(.orange)
                                        Text(NSLocalizedString("cost.default_rate", comment: ""))
                                            .font(.caption2).foregroundColor(.secondary)
                                    }
                                }
                                Spacer()
                                Text("\(loc.pricePerKwh.formatted(.currency(code: Locale.current.currency?.identifier ?? "USD")))/kWh")
    ...
    func loadData() async {
        ...
        // Location ranking with TOU cost
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss"
        dateFormatter.timeZone = TimeZone.current
        var locMap: [String: (cost: Double, kWh: Double, count: Int, touCost: Double)] = [:]
        for c in charges {
            var entry = locMap[c.address] ?? (0, 0, 0, 0)
            entry.cost += c.cost
            entry.kWh += c.chargeEnergyAdded
            entry.count += 1
            let hour: Int
            if let date = dateFormatter.date(from: String(c.startDate.prefix(19))) {
                hour = Calendar.current.component(.hour, from: date)
            } else {
                hour = 12
            }
            entry.touCost += c.chargeEnergyAdded * tariffConfig.priceForHour(hour)
            locMap[c.address] = entry
        }
        ranking = locMap.map { addr, v in
            LocationCost(address: addr, cost: v.cost, kWh: v.kWh,
                         pricePerKwh: (v.cost / max(v.kWh, 0.1) * 100).rounded() / 100,
                         count: v.count, touCost: (v.touCost * 100).rounded() / 100)
