...
const list = (m.cars as any[]).map(flattenCar);
...
export const useStore = create<Store>((set)=>({
  currentCarId: 1, cars: list, theme: 'system', mockMode: true,
  serverUrl: '', apiToken: '', onboardingDone: true,
  setCarId: (id)=>set({currentCarId:id}),
  setTheme: (t)=>{set({theme:t}); document.documentElement.classList.toggle('dark',t==='dark'||(t==='system'&&window.matchMedia('(prefers-color-scheme:dark)').matches));},
